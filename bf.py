...
SAMBACKUS_REGEX = "(sambackus[\.\!\?])\s*(sambackus[\.\!\?])"

def convertSambackusToBF(bf_input):
    output = []
    for match in re.findall(SAMBACKUS_REGEX, bf_input.lower()):
        if match in sambackus2bf:
            output.append(sambackus2bf[match])
    return "".join(output)
...
if __name__ == '__main__':
    ...
    if sys.argv[1] == "-o":
        listing = convertSambackusToBF(listing)
